...
- [ ] D-5-1: Create `client/src/components/SearchBar.jsx` — controlled text input, props: `value`, `onChange`, placeholder `"Search tasks..."`
...
- [ ] D-5-7: Render `<FilterBar />` in `App.jsx` above the board in the tasks view
- [ ] D-5-8: Add no-results state to `KanbanBoard.jsx` — when `todos.length === 0` render `<div className="empty-state"><p>No tasks match your filters.</p></div>` before the `DndContext`
